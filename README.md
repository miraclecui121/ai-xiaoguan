...
Postgres 是增强能力，不是应用启动前置条件。若 Render Postgres 到期、暂停或连接失败，服务会自动降级：

- 仍可演示：打开首页、微信登录流程、默认演示数据、客户/联系人/商机/跟进/日程等本地数据操作、AI 专家对话、DeepSeek 调用、豆包联网检索、GLM 视觉识别。
- 暂不可用或不可靠：PC/微信跨端云同步、运营日志持久查询、历史对话云端恢复、邀请码发放/激活的长期追踪。
- 恢复方式：升级/恢复 Render Postgres，或创建新 Postgres 后把 Web Service 的 `DATABASE_URL` 替换为新连接串，再重新部署/重启服务即可。
