...
- 2026-08-09：AI 助手新增“客户回复后，值不值得跟”场景，对应用户级 `~/.workbuddy/skills/lead-judgment-assistant/` 的线索判断助手方法论，统一走 `lead-dev` 专家视角，强调意向水温、缺失信息、白嫖/比价/拖延风险、下一问和可直接发送回复。对话框新增图片入口，当前仅支持本轮临时图片附件和文件名上下文，不保存图片到云端历史；Excel/PDF 暂不接入，后续应先评估 OCR/视觉模型与隐私策略。上线前已创建回滚标签 `rollback/before-ai-guided-workflow-20260809`。
